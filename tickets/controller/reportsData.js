...
}; // Count of all tickets including opened.

const ticketsAvgRating = async function () {
	// Rating of all closed Tickets which were rated.
			...
			(avgRating =
				solvedTickets.length > 0
					? solvedTickets.reduce(
							(sum, ticket) => sum + (ticket.rating != null ? ticket.rating * 1.0 : 5.0),
							0
...
const agentOpenTickets = async function (req) {
	...
	const agentWithMostOpenedTickets = agentsAndTickets.reduce(
		(maxAgent, currentAgent) => {
...
const agentSolvedTickets = async function (req) {
	...
	const agentWithMostSolvedTickets = agentsAndTickets.reduce(
		(maxAgent, currentAgent) => {
...
exports.generateAgentPerformanceReport = async (req, res) => {
	...
	res.status(200).json({
		...
		data: {
			agents: agents,
			numberOfTickets: countOfTickets,
			averageRating: avgRateAllTickets,
			agentWithMostOpenedTickets: mostOpenedAgent,
			agentWithMostSolvedTickets: mostSolvedAgent,
